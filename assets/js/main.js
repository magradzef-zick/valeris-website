/* =================================================================
   VALERIS — Behaviour
   Language switch · sticky nav + scroll-spy · mobile menu ·
...
(function () {
  ...
  /* ---------- language ----------
     Each language is a real static page (EN at /page, PL at /pl/page — generated by
     tools/build-pl.mjs), so the switcher simply navigates to the counterpart URL. */
  var LANG = document.documentElement.lang === 'pl' ? 'pl' : 'en';
  function counterpartUrl(lang) {
    var path = location.pathname.replace(/index\.html$/, '');
    var base = path.replace(/^\/pl(\/|$)/, '/');
    return (lang === 'pl' ? '/pl' + base : base) + location.hash;
  }
  document.querySelectorAll('.lang-btn').forEach(function (b) {
    var lang = b.getAttribute('data-lang');
    var active = lang === LANG;
    b.classList.toggle('is-active', active);
    b.setAttribute('aria-pressed', active ? 'true' : 'false');
    b.addEventListener('click', function () { if (!active) location.href = counterpartUrl(lang); });
  });
  window.__valerisT = function (key) { return (DICT[LANG] || DICT.en)[key] || (DICT.en[key] || key); };
  ...
  if (hamburger && navLinks) {
    var closeMobileMenu = function () {
      ...
      hamburger.setAttribute('aria-label', LANG === 'pl' ? 'Otwórz menu' : 'Open menu');
    ...
    hamburger.addEventListener('click', function () {
      ...
      hamburger.setAttribute('aria-label', LANG === 'pl' ? (open ? 'Zamknij menu' : 'Otwórz menu') : (open ? 'Close menu' : 'Open menu'));
  ...
  if (statNums.length && !reduceMotion) {
    var countUp = function (el) {
      ...
      var suffix = el.textContent.replace(/^\d+/, '');   /* keep e.g. the '+' in '30+' */
      var start = null;
      var duration = 700;
      function tick(ts) {
  ...
  /* ---------- page transition between site pages (200ms fade) ---------- */
  var thisPage = location.pathname.split('/').pop() || 'index.html';
  document.querySelectorAll('a[href]').forEach(function (a) {
    var href = a.getAttribute('href');
    if (!href || href.charAt(0) === '#' || href.indexOf(':') !== -1) return;
    var hrefPage = href.split('#')[0].split('?')[0].split('/').pop() || 'index.html';
    if (!/\.html$/.test(hrefPage)) return;
